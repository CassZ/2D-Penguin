...
void drawLeg();
void drawFoot();
void render(int renderStyle, GLfloat color1[][3], GLfloat c2[], int bodypart);
void calculate_normals(GLfloat bodypart_coord[][3], GLfloat calculated[][3]);
void normal(GLfloat p0[], GLfloat p1[], GLfloat p2[], GLfloat *n_x, GLfloat *n_y, GLfloat *n_z);
enum {_BODY, _HEAD, _BEAK, _ARM, _HAND, _LEG, _FOOT};
...
void initGlui()
{
	...
	//panel for control of light source
	glui_panel = glui_render->add_panel("Light Spin");
	glui_spinner = glui_joints->add_spinner_to_panel(glui_panel, "Light:",
	GLUI_SPINNER_FLOAT, &light_spin);
	...
	//added control for metallic and matte
...
Vector getInterpolatedJointDOFS(float time)
{
	...
	if (i == 1) // special case - at beginning of spline
	{
		...
		t1
				= (keyframes[i + 1].getDOFVector()
						- keyframes[i - 1].getDOFVector()) * 0.5;
	} else if (i == maxValidKeyframe) // special case - at end of spline
	{
		t0 = (keyframes[i].getDOFVector() - keyframes[i - 2].getDOFVector())
				* 0.5;
...
void display(void)
{
	...
	render(renderStyle, mycolor1, mycolor2, _BODY);
		glPushMatrix(); //push head
		glRotatef(joint_ui_data->getDOF(Keyframe::HEAD),
				0.0, 1.0, 0.0);
		glTranslatef(0, 1.8, 0);
		render(renderStyle, mycolor1, mycolor2, _HEAD);
			glPushMatrix(); //push upper beak
			glTranslatef(-0.7-0.3, 0, 0);
			render(renderStyle, mycolor1, mycolor2, _BEAK);
				glPushMatrix(); //push lower beak
				glTranslatef(0, -0.05, 0);
				glTranslatef(0, -joint_ui_data->getDOF(Keyframe::BEAK), 0);
				render(renderStyle, mycolor1, mycolor2, _BEAK);
				glPopMatrix(); //pop lower beak
			glPopMatrix(); //pop upper beak
		...
		render(renderStyle, mycolor1, mycolor2, _ARM);

			glPushMatrix(); //push left hand
			glTranslatef(0, -0.4, 0);
			glRotatef(joint_ui_data->getDOF(Keyframe::L_ELBOW), 0.0, 1.0, 0.0);
			glTranslatef(0, -0.4, 0);
			render(renderStyle, mycolor1, mycolor2, _HAND);
			glPopMatrix();
		glPopMatrix(); //pop left arm
		...
		render(renderStyle, mycolor1, mycolor2, _ARM);
			...
			glPushMatrix(); //push right hand
			glTranslatef(0, -0.4, 0);
			glRotatef(joint_ui_data->getDOF(Keyframe::R_ELBOW), 0.0, 1.0, 0.0);
			glTranslatef(0, -0.4, 0);
			render(renderStyle, mycolor1, mycolor2, _HAND);
			glPopMatrix();
		glPopMatrix(); //pop right arm

		glPushMatrix(); //push left leg
		glTranslatef(0, -0.5, 0.03+0.2);
		...
		render(renderStyle, mycolor1, mycolor2, _LEG);
			glPushMatrix();
			glTranslatef(0.06, -0.5, 0);
			glRotatef(joint_ui_data->getDOF(Keyframe::L_KNEE),
				0.0, 0.0, 1.0);
			glTranslatef(-0.3, 0, 0);
			render(renderStyle, mycolor1, mycolor2, _FOOT);
			glPopMatrix();
		glPopMatrix();

		glPushMatrix(); //push right leg
		glTranslatef(0, -0.5, -0.03-0.2);
...
// Draw body parts, centered at the current location
// README: Helper code for drawing body parts
GLfloat 
body_coord[8][3] = {
	{-1.0, -1.5, 0.7}, //front left bottom
	{1.0, -1.5, 0.7}, //front right bottom
	{0.7, 1.5, 0.7}, //front right top
	{-0.7, 1.5, 0.7}, //front left top
	{-1.0, -1.5, -0.7}, //back left bottom
	{1.0, -1.5, -0.7}, //back right bottom
	{0.7, 1.5, -0.7}, //back right top 
	{-0.7, 1.5, -0.7} //back left top
};
void drawBody() {
	GLfloat normals[6][3] = {{0}};
	calculate_normals(body_coord, normals);
	glBegin(GL_QUADS);
	//draw front face
	glNormal3fv(normals[0]);
	glVertex3f(-1.0, -1.5, 0.7);
	glVertex3f(1.0, -1.5, 0.7);
	glVertex3f(0.7, 1.5, 0.7);
	glVertex3f(-0.7, 1.5, 0.7);
...
GLfloat 
head_coord[8][3] = {
	{-0.78, -0.4, 0.78}, //front left bottom
	{0.78, -0.4, 0.78}, //front right bottom
	{0.70, 0.4, 0.70}, //front right top
	{-0.70, 0.4, 0.70}, //front left top
	{-0.78, -0.4, -0.78}, //back left bottom
	{0.78, -0.4, -0.78}, //back right bottom
	{0.70, 0.4, -0.70}, //back right top 
	{-0.70, 0.4, -0.70} //back left top
};
void drawHead() {
	GLfloat normals[6][3] = {{0}};
	calculate_normals(head_coord, normals);
	glBegin(GL_QUADS);
	//draw front face
	glNormal3fv(normals[0]);
	glVertex3f(-0.78, -0.4, 0.78);
	glVertex3f(0.78, -0.4, 0.78);
	glVertex3f(0.70, 0.4, 0.70);
	glVertex3f(-0.70, 0.4, 0.70);
...
GLfloat 
beak_coord[8][3] = {
	{-0.3, -0.05, 0.05}, //front left bottom
	{0.3, -0.05, 0.05}, //front right bottom
	{0.3, 0.05, 0.05}, //front right top
	{-0.3, 0.05, 0.05}, //front left top
	{-0.3, -0.05, -0.05}, //back left bottom
	{0.3, -0.05, -0.05}, //back right bottom
	{0.3, 0.05, -0.05}, //back right top 
	{-0.3, 0.05, -0.05} //back left top
};
void drawBeak(){
	GLfloat normals[6][3] = {{0}};
	calculate_normals(beak_coord, normals);
	glBegin(GL_QUADS);
	//draw front face
	glNormal3fv(normals[0]);
...
GLfloat 
arm_coord[8][3] = {
	{-0.2, -0.7, 0.05}, //front left bottom
	{0.2, -0.7, 0.05}, //front right bottom
	{0.4, 0.7, 0.05}, //front right top
	{-0.4, 0.7, 0.05}, //front left top
	{-0.2, -0.7, -0.05}, //back left bottom
	{0.2, -0.7, -0.05}, //back right bottom
	{0.4, 0.7, -0.05}, //back right top
	{-0.4, 0.7, -0.05} //back left top
};
void drawArm(){
	GLfloat normals[6][3] = {{0}};
	calculate_normals(arm_coord, normals);
	glBegin(GL_QUADS);
	//draw front face
	glNormal3fv(normals[0]);
	glVertex3f(-0.2, -0.7, 0.05);
	glVertex3f(0.2, -0.7, 0.05);
	glVertex3f(0.4, 0.7, 0.05);
	glVertex3f(-0.4, 0.7, 0.05);
...
void drawHand(){
	GLfloat normals[6][3] = {{0}};
	calculate_normals(hand_coord, normals);
	glBegin(GL_QUADS);
	//draw front face
	glNormal3fv(normals[0]);
...
void drawLeg(){
	GLfloat normals[6][3] = {{0}};
	calculate_normals(leg_coord, normals);
	glBegin(GL_QUADS);
	//draw front face
	glNormal3fv(normals[0]);
	glVertex3f(-0.03, -0.5, 0.03);
	glVertex3f(0.03, -0.5, 0.03);
	glVertex3f(0.03, 0.5, 0.03);
	glVertex3f(-0.03, 0.5, 0.03);
...
void drawFoot(){
	GLfloat normals[6][3] = {{0}};
	calculate_normals(foot_coord, normals);
	glBegin(GL_QUADS);
	//draw Front
	glNormal3fv(normals[0]);
	glVertex3f(0.3, 0.03, 0);
	glVertex3f(-0.3, 0.03, 0.2);
	glVertex3f(-0.3, -0.03, 0.2);
	glVertex3f(0.3, -0.03, 0);
	//draw back
	glNormal3fv(normals[1]);
...
//render function, draw body parts according render style
void render(int renderStyle, GLfloat color[][3], GLfloat color2[], int mybodypart){
	...
	else if (renderStyle == MATTE || renderStyle == METALLIC){
		glShadeModel(GL_FLAT);
		if (renderStyle == MATTE){ //high diffuse low specular
			glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
			GLfloat ambient[] = {0.1, 0.1, 0.1, 1};
			GLfloat diffuse[] = {0.41, 0.41, 0.41, 1};
...
//normal computing helper
void calculate_normals(GLfloat bodypart_coord[][3], GLfloat calculated[][3]){
	int i;
	for (i=0; i < 6; i++){
		if(i == 0){
			normal(bodypart_coord[0], bodypart_coord[1], bodypart_coord[2], &calculated[i][0], &calculated[i][1], &calculated[i][2]);
		}
		else if (i == 1){
			normal(bodypart_coord[6], bodypart_coord[5], bodypart_coord[4], &calculated[i][0], &calculated[i][1], &calculated[i][2]);
		}
		else if (i == 2){
			normal(bodypart_coord[0], bodypart_coord[3], bodypart_coord[4], &calculated[i][0], &calculated[i][1], &calculated[i][2]);
		}
		else if (i == 3){
			normal(bodypart_coord[6], bodypart_coord[2], bodypart_coord[1], &calculated[i][0], &calculated[i][1], &calculated[i][2]);
		}
		else if (i == 4){
			normal(bodypart_coord[6], bodypart_coord[3], bodypart_coord[2], &calculated[i][0], &calculated[i][1], &calculated[i][2]);
		}
		else if (i == 5){
			normal(bodypart_coord[4], bodypart_coord[1], bodypart_coord[0], &calculated[i][0], &calculated[i][1], &calculated[i][2]);
		}
	}
}
...
void normal(GLfloat p0[], GLfloat p1[], GLfloat p2[], GLfloat *n_x, GLfloat *n_y, GLfloat *n_z){
	...
	*n_x = e0[1]*e1[2] - e0[2]*e1[1];
	*n_y = e0[2]*e1[0] - e0[0]*e1[2];
	*n_z = e0[0]*e1[1] - e0[1]*e1[0];
}
